<?php
  ...
  $page_title = 'Accept Blood Reject!';
  ?>
...
<?php  
  ...
  require_once('body.php');
	...
	$query = "update blood_management.donation_event_tbl set STATUS='R',L_CHG_USER_ID='$user_id'
...
if ($result) {
    commit($dbc);
	$msg = 'DONATION ID:  ' . $don_id . ' has been Rejected.';
    echo '<script>';
    ...
    rollback($dbc);
	$err_msg = 'Oops! Something went wrong while trying to Rejecting ' . $don_id . ' Donation ID. Please try again later or contact support for assistance.';
}
	mysqli_close($dbc);  
