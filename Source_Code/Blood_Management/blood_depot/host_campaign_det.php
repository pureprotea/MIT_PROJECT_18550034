...
$l_chg_user_id = $row['L_CHG_USER_ID'];
	...
	//if invalid bd_id no keyed in through url redirects to add_book.php
	else{
...
if(!empty($bd_id) && !empty($location)&& !empty($sdate)&& !empty($edate))
{
...
//insert minimum once or until no of insert times reaches to No of Book Count(add_count)
	...
	$query = "INSERT INTO blood_donation_campaign_tbl (BD_ID,START_DATE,END_DATE,LOCATION,R_CRE_USER_ID,L_CHG_USER_ID) 
			values ('$bd_id',  STR_TO_DATE('$sdate','%Y-%m-%d'), STR_TO_DATE('$edate','%Y-%m-%d'),'$location','$user', '$user')";
...
if ($result) {
    commit($dbc);
    $msg = 'Campaign details successfully added';
    echo '<script>';
    echo 'alert("' . $msg . '");'; // Display an alert box with the success message
    echo 'window.location.href = "http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/index.php";'; 
    echo '</script>';
    exit; // exit the script after redirection
	}
	else {
    rollback($dbc);
    $err_msg = 'Campaign details not added successfully';
	}
...
?>
    ...
    <form name="addbookform" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
  ...
  <div class="left" style="width:60%">
	 <h2>Campaign Details</h2>
	  ...
	  <div class="field">
        ...
        <input id="BRANCH" name="BRANCH" type="text" size="45" value="Head Office" readonly="readonly" />
      </div>
	  ...
	  <div class="button">
		<input type="submit" class="sumbit" value="Host Campaign" name="submit" />
	  </div>
...
<div class="right" style="width:40%">
	...
	<div>
		<a href='index.php' style="display: inline-block; margin-top: 130px;float:right;width:46.5px">
			<img id='button_right' src='images/back.png' alt='BACK'/>
		</a>
	</div>
	
	<!--  <div class="button" style="display: inline-block; margin-top: 115px;float:right">
		<input type="submit" class="sumbit" value="Host Campaign" name="submit" />
	  </div> -->
</div>
