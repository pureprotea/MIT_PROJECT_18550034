...
<title>Web Based Blood Management System - <?php echo $page_title?> </title>
   ...
   <script type="text/javascript" src="validation.js"></script>
	...
	<style>
      ...
      div.body {
         background-image: url("body_new.gif");
      }
   </style>
...
$r_cre_bd_id = $row['R_CRE_USER_ID'];
$l_chg_time = $row['L_CHG_TIME'];
$l_chg_bd_id = $row['L_CHG_USER_ID'];
...
// Grab the POST data
$bd_id = mysqli_real_escape_string($dbc, trim($_POST['USER__HIDDEN_ID']));
...
$r_cre_bd_id = mysqli_real_escape_string($dbc, trim($_POST['R_CRE_BD_ID_HIDDEN_ID']));
$l_chg_time = mysqli_real_escape_string($dbc, trim($_POST['L_CHG_TIME_HIDDEN_ID']));
$l_chg_bd_id = mysqli_real_escape_string($dbc, trim($_POST['L_CHG_BD_ID_HIDDEN_ID']));
...
?>
	...
	<div class="left">
      ...
      <div class="field">
        ...
        <input id="USER__HIDDEN_ID" name="USER__HIDDEN_ID" type="hidden" size="12" 
	  ...
	  <div class="field">
		...
		value="<?php if (!empty($r_cre_time)) echo $r_cre_time; ?>" />
	        ...
	        <div class="field">
        <label for="R_CRE_BD_ID">Record Created User:</label>
        <input id="disabled" name="R_CRE_BD_ID" type="text" size="20" 
		value="<?php if (!empty($r_cre_bd_id)) echo $r_cre_bd_id; ?>"  disabled="true" />
      </div>
	  
	  		<!--disabled tag fields not getting submitted on form submission, so hidden tag is used -->
        <input id="R_CRE_BD_ID_HIDDEN_ID" name="R_CRE_BD_ID_HIDDEN_ID" type="hidden" size="20" 
		value="<?php if (!empty($r_cre_bd_id)) echo $r_cre_bd_id; ?>" />
		...
		value="<?php if (!empty($l_chg_time)) echo $l_chg_time; ?>" />
	        ...
	        <div class="field">
        <label for="L_CHG_BD_ID">Last Modified User:</label>
        <input id="disabled" name="L_CHG_BD_ID" type="text" size="20" 
		value="<?php if (!empty($l_chg_bd_id)) echo $l_chg_bd_id; ?>"  disabled="true" />
      </div>
	  
	  		<!--disabled tag fields not getting submitted on form submission, so hidden tag is used -->
        <input id="L_CHG_BD_ID_HIDDEN_ID" name="L_CHG_BD_ID_HIDDEN_ID" type="hidden" size="20" 
		value="<?php if (!empty($l_chg_bd_id)) echo $l_chg_bd_id; ?>" />
		<br/>
		<br/>
		<br/>
	  	<br/>
