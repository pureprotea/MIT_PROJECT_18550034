...
if(!empty($user_id))
{
$query = "SELECT USER_ID,DONOR_STATUS from user_master_tbl WHERE user_id='$user_id' and del_flg='N'";
...
{
$query = "SELECT USER_ID,DONOR_STATUS from user_master_tbl WHERE id_num='$id_num' and del_flg='N'";
...
//valid membership id found redirects to user_status_det details page				
if(!empty($valid_user_id)){
          $url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . 
		  '/user_status_det.php?user_id=' . $valid_user_id.'&status='.$user_status;
          header('Location: ' . $url);      
		}
		else {
...
?>
  <form name="searchmemberform" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
	...
	<div class="field" style="padding-top:7%">
        <label for="USER_ID">User Ref:</label>
	...
	<div class="center" style="padding-top:8%;">
		<input type="submit" class="sumbit" value="Find User" name="submit" />
	  </div>
