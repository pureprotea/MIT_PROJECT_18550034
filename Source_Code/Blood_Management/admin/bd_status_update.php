...
if(!empty($bd_id))
{
$query = "SELECT BD_ID,STATUS from blood_depot_master_tbl WHERE BD_ID='$bd_id'";
...
if(!empty($valid_bd_id)){
		  ...
		  '/bd_status_update_det.php?bd_id=' . $valid_bd_id.'&status='.$status;
...
?>
  <form name="searchmemberform" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
  	...
  	<div class="center">
		...
		<div class="field">
			<input type="submit" class="sumbit" value="Manage BD" name="submit" />
		</div>
	</div>
