<?php
  ...
  $page_title = 'Blood Depot Activation!';
  ?>
...
if ($status=='A'){					
 $err_msg="Blood Depot is Already in Active Status";
...
begin($dbc);
// update database
$query = "update blood_depot_master_tbl set STATUS='A', L_CHG_TIME=now(), L_CHG_USER_ID='$user'
			 where BD_ID='$bd_id'";
...
if ($result){
	commit($dbc);
	$err_msg="Blocked Blood Depot has been successfully Reactivated";
	$status='A';
...
$query1 = "INSERT INTO blood_depot_login_tbl (BD_ID, USER_PWD, NO_OF_ATTEMPTS, R_CRE_TIME, R_CRE_USER_ID)
			values ('$bd_id', sha(CONCAT(UPPER('$bd_id'),'$password')), '0',now(), '$user')";
...
$status='A';
	...
	//if invalid blood depot id keyed in through url redirects to memberstatus.php
	else{
	 	$url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . 
		 '/bd_status_update.php';
         header('Location: ' . $url);
		}
...
?>
	...
	<div class="right">
	  ...
	  <div class="field">
        ...
        <input id="green" name="STATUS" type="text" size="12" value="<?php if (!empty($m_status)) echo $m_status; ?>"
		 readonly="readonly" />
...
<a href='bd_status_update_det.php?bd_id=<?php echo $bd_id;?>&status=<?php echo $status;?>'><img id='button_right' src='/blood_management/admin/images/back.png' alt='BACK'/></a>
