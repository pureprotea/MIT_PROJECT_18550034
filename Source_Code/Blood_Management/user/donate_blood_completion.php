...
<?php  
  ...
  require_once('body.php');
	...
	$query = "update blood_management.donation_event_tbl set STATUS='A',L_CHG_USER_ID='$user_id'
...
if ($result) {
    commit($dbc);
	$msg = 'DONATION ID:  ' . $don_id . ' has been Agreed by Donor.';
    echo '<script>';
    echo 'alert("' . $msg . '");'; // Display an alert box with the success message
    echo 'window.location.href = "http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/Questionarie.php";'; 
    echo '</script>';
