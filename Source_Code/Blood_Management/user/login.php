  ...
  if (!isset($_SESSION['user_id'])) {
    if (isset($_POST['submit'])) {
      ...
      if (!empty($user_username) && !empty($user_password)) {
        ...
        $query = "SELECT a.user_id, b.first_name,a.image_name FROM user_login_tbl a,user_master_tbl b WHERE a.user_id=b.user_id and 
					  a.user_id = '$user_username' AND a.user_pwd = sha(CONCAT(UPPER('$user_username'),'$user_password'))and b.del_flg='N'";
        ...
        if (mysqli_num_rows($results) == 1) {
          ...
          setcookie('user_id', $row['user_id'], time() + (60 * 60 * 24 * 45));    // expires in 45 days
          setcookie('first_name', $row['first_name'], time() + (60 * 60 * 24 * 45));  // expires in 45 days
          $home_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/index.php';
          header('Location: ' . $home_url);
        }
